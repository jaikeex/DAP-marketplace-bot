...
BASE_URL = "https://moznosti.dantem.net"
...
def scan_and_accept(session: requests.Session,
    ...
    if is_empty(scan_response):
        ...
        if accept:
            parse_and_accept_offers(session, shifts, dates)
        else:
            parse_and_report_offers(shifts, dates)


def parse_and_accept_offers(session: requests.Session,
                            shifts: list,
                            dates: list):
    now = datetime.datetime.today().strftime('%H:%M:%S')
    for shift in shifts:
...
def parse_and_report_offers(shifts: list, dates: list):
    now = datetime.datetime.today().strftime('%H:%M:%S')
    for shift in shifts:
        if is_valid_offer(shift, dates):
...
def parse_market(response: requests.models.Response.text) -> list:
    ...
    for i in range(nr_offers):
        ...
        place_index = divs.index(time_div) + 2
        place = divs[place_index].text.split("\n")[2].strip()[:-1]

        link_index = divs.index(time_div) + 3
